...
class BarcodeGenerator:
    ...
    def generate_booking_barcode(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Fallback barcode generation failed: {str(fallback_error)}"
                )
    ...
    def generate_code39_barcode(order_id: str) -> str:
        ...
        try:
            ...
            filtered_id = "".join(
                c for c in order_id.upper() if c in valid_chars
            )

            if not filtered_id:
                filtered_id = order_id.upper()[
                    :10
                ]  # Take first 10 chars as fallback
    ...
    def prepare_booking_info_for_display(
        ...
        return {
            ...
            "booking": {
                ...
                "total_seats": sum(
                    cat.get("num_seats", 0) for cat in seat_categories
                ),
                "seat_categories": [
                    {
                        "label": cat.get("label", ""),
