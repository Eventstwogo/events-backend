...
class SessionManager:
    ...
    async def terminate_all_user_sessions(
        ...
        try:
            ...
            if except_session_id:
                stmt = stmt.where(AdminUserDeviceSession.session_id != except_session_id)
    ...
    async def get_user_sessions(
        ...
        """
        try:
            stmt = select(AdminUserDeviceSession).where(AdminUserDeviceSession.user_id == user_id)

            if active_only:
                stmt = stmt.where(AdminUserDeviceSession.is_active == True)

            stmt = stmt.order_by(desc(AdminUserDeviceSession.last_used_at)).limit(limit)
    ...
    @staticmethod
    async def _get_active_sessions(user_id: str, db: AsyncSession) -> List[AdminUserDeviceSession]:
