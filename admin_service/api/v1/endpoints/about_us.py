...
@exception_handler
async def create_or_update_about_us(
    about_us_data: AboutUsCreateRequest,
    db: AsyncSession = Depends(get_db),
) -> JSONResponse:
    """
    Create or update the single About Us record.

    Args:
        about_us_data: The about us data to create or update
        db: Database session

    Returns:
        JSONResponse: The created or updated about us details
    """

    # Check if About Us record already exists
    existing_stmt = select(AboutUs).limit(1)
    result = await db.execute(existing_stmt)
    existing_about_us = result.scalar_one_or_none()

    if existing_about_us:
        # Update existing record
        existing_about_us.about_us_data = about_us_data.about_us_data
        await db.commit()
        await db.refresh(existing_about_us)
        return api_response(
            status_code=status.HTTP_200_OK,
            message="About Us record updated successfully",
            data=AboutUsResponse.model_validate(existing_about_us),
        )

    # Create new About Us record if none exists
    about_us_id = generate_digits_upper_lower_case(6)

    # Ensure ID is unique
    while True:
        existing_id_stmt = select(AboutUs).where(
            AboutUs.about_us_id == about_us_id
        )
        id_check = await db.execute(existing_id_stmt)
        if not id_check.scalar_one_or_none():
            break
        about_us_id = generate_digits_upper_lower_case(6)
...
async def get_about_us(
    ...
    """
    Get the single About Us record.

    Args:
        db: Database session

    Returns:
        JSONResponse: The About Us record if found
    """
    stmt = select(AboutUs).limit(1)
    result = await db.execute(stmt)
    about_us_record = result.scalar_one_or_none()
    ...
    return api_response(
        ...
        data=AboutUsResponse.model_validate(about_us_record),
    )


@router.put("", response_model=AboutUsResponse)
@exception_handler
async def update_about_us(
    ...
    """
    # Fetch the single About Us record
    stmt = select(AboutUs).limit(1)
    result = await db.execute(stmt)
