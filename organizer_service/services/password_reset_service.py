...
from shared.db.models import AdminUser, PasswordReset
...
async def create_password_reset_record(
    ...
    Returns:
        PasswordReset: The created/updated password reset record
    """
    try:
...
async def get_password_reset_by_token(
    ...
    Returns:
        PasswordReset or None: The password reset record if found
    """
    try:
        stmt = (
            select(PasswordReset)
            .options(selectinload(PasswordReset.user))
            .where(PasswordReset.reset_password_token == token)
        )
        result = await db.execute(stmt)
...
async def cleanup_expired_tokens(db: AsyncSession) -> int:
    ...
    try:
        ...
        stmt = select(PasswordReset).where(
            PasswordReset.reset_token_expires_at < current_time,
            PasswordReset.reset_password_token.is_not(None),
        )
        result = await db.execute(stmt)
