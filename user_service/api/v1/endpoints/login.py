...
async def process_successful_login(
    ...
    if suspicious_alerts:
        logger.warning(
            f"Suspicious activity detected for user {user.user_id}: {suspicious_alerts}"
        )
                        ...
                        for session in active_sessions:
                            if session.ip_address == ip_address:
                                ...
                                logger.info(
                                    ...
                                    f""
                                    f"for user {user_id} by IP match"
                                )
                                break

                        if not session_found:
                            logger.warning(
                                f"No matching session found for user {user_id} "
                                f""
                                f"with IP {ip_address}"
                            )
                    else:
                        logger.warning(
                            f"No IP address available for fallback session "
                            f"termination for user {user_id}"
                        )
